...
const COMMAND_GROUPS = [
  {
    ...
    commands: [
      { cmd: 'MENU',         desc: 'Opens the main menu with all available options.' },
      { cmd: 'LISTINGS',     desc: 'Shows all active listings across every category.' },
      { cmd: 'VIEW [ID]',    desc: 'View full details for a specific listing. Replace [ID] with the listing code, e.g. ADS-4821.' },
    ],
  },
  {
    id: 'buy', label: 'Buy', icon: ICONS.lock,
    commands: [
      { cmd: 'BUY [ID]',             desc: 'Start a purchase. Funds go into escrow until you confirm access.' },
      { cmd: 'OFFER [ID]',           desc: 'Make an offer below the listed price. Seller has 72 hours to respond.' },
      { cmd: 'ACCEPT [ID]',          desc: 'Accept a counter-offer from the seller.' },
      { cmd: 'REJECT [ID]',          desc: 'Decline a counter-offer without obligation.' },
      { cmd: 'COUNTER [ID] [amt]',   desc: 'Send back a counter with your price. Replace [amt] with your offer amount.' },
      { cmd: 'CANCEL OFFER [ID]',    desc: 'Withdraw your offer before the seller responds.' },
    ],
  },
  {
    id: 'sell', label: 'Sell', icon: ICONS.dollar,
    commands: [
      { cmd: 'SELL',       desc: 'Start the listing flow — guided step by step to submit your asset.' },
      { cmd: 'MY OFFERS',  desc: 'See all incoming offers on your active listings.' },
    ],
  },
  {
    id: 'requests', label: 'Requests', icon: ICONS.inbox,
    commands: [
      { cmd: 'REQUEST',                  desc: "Post a public request for an asset you're looking for. Sellers can respond." },
      { cmd: 'MY REQUESTS',             desc: "View all requests you've posted and their current status." },
      { cmd: 'RESPOND [REQ-ID]',        desc: "Reply to a buyer's request as a seller. Replace [REQ-ID] with the request code." },
      { cmd: 'CANCEL REQUEST [REQ-ID]', desc: 'Remove a request you posted.' },
    ],
  },
  {
    id: 'settings', label: 'Settings', icon: ICONS.shield,
    commands: [
      { cmd: 'HELP',               desc: 'Get the full command list sent directly in WhatsApp.' },
      { cmd: 'NOTIFICATIONS ON',   desc: 'Turn on alerts for new listings, offers, and status updates.' },
      { cmd: 'NOTIFICATIONS OFF',  desc: 'Pause all notifications. You can still trade normally.' },
    ],
  },
];
      ...
      <div className="container">
        ...
        <div style={{ marginTop: 72 }}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: `1px solid ${isActive ? 'var(--forest)' : 'var(--border)'}`,
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                      ...
                      <span style={{
                        ...
                        background: isOpen ? 'var(--forest)' : 'var(--border-dk)',
                        transition: 'background 0.15s',
                      }} />
                      <span style={{
                        fontFamily: "'Courier New', monospace",
          ...
          <div style={{
            ...
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--ink-3)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: 1 }}>
              <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
            </svg>
            <p style={{ fontSize: '0.78rem', color: 'var(--ink-3)', lineHeight: 1.6, margin: 0 }}>
              Replace{' '}
              <code style={{ fontFamily: 'monospace', fontSize: '0.78rem', background: 'var(--off)', padding: '1px 5px', borderRadius: 4 }}>[ID]</code>
              {' '}and{' '}
              <code style={{ fontFamily: 'monospace', fontSize: '0.78rem', background: 'var(--off)', padding: '1px 5px', borderRadius: 4 }}>[amt]</code>
              {' '}with the actual listing ID or value — e.g.{' '}
              <code style={{ fontFamily: 'monospace', fontSize: '0.78rem', background: 'var(--off)', padding: '1px 5px', borderRadius: 4 }}>ADS-4821</code>.
            </p>
          </div>
        </div>
      </div>
